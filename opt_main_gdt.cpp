...
void step() {
    ...
    for(size_t i=0; i<len; i++){
        ...
        //     d2s[j] = dxs[j] * dxs[j] + dys[j] * dys[j] + dzs[j] * dzs[j] + eps * eps;
        //     ivf_d2s[j] = 1.0 / (d2s[j] * std::sqrt(d2s[j]));
        //     stars.vxs[i] += dxs[j] * stars.masses[j] * (G * dt * ivf_d2s[j]);
        //     stars.vys[i] += dys[j] * stars.masses[j] * (G * dt * ivf_d2s[j]);
        //     stars.vzs[i] += dzs[j] * stars.masses[j] * (G * dt * ivf_d2s[j]);
        // }
        ...
        #pragma opm simd
        for(size_t j=0; j<len; j++){
            ivf_d2s[j] = gdt * 1.0 / (d2s[j] * std::sqrt(d2s[j]));
        }
        #pragma opm simd
        for(size_t j=0; j<len; j++){
            stars.vxs[i] += dxs[j] * stars.masses[j] * ( ivf_d2s[j]);
        }
        #pragma opm simd
        for(size_t j=0; j<len; j++){
            stars.vys[i] += dys[j] * stars.masses[j] * ( ivf_d2s[j]);
        }
        #pragma opm simd
        for(size_t j=0; j<len; j++){
            stars.vzs[i] += dzs[j] * stars.masses[j] * ( ivf_d2s[j]);
        }
    }
    // #pragma opm simd
    // for(size_t i=0; i<len; i++){
